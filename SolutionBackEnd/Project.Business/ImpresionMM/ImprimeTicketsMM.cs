...
namespace Milano.BackEnd.Business.ImpresionMM
{
    ...
    public class ImprimeTicketsMM : BaseBusiness
    {
        ...
        {
            Funciones = "Imprime ImprimeCentrado ImprimeCodigoDeBarras ImprimeCompleto ImprimeImagen PonFuente PonNegrita QuitaNegrita PonCursiva QuitaCursiva PonSubrayado ";
            ...
            Funciones += "AbrirCajon AvanzarNCaracteres AvanzarNLineas RellenarNConX SubCadena Concatenar";
        ...
        private void ProcesaVoucher(DataTable voucher)
        {
            ...
            if (voucher.Rows[i]["EsBancomerVB"].ToString() == "1")
            {
                ...
                RealizarAccion("");
                if (voucher.Rows[i]["numeroTarjetaVB"].ToString().Length == 4)
                {
                    if (voucher.Rows[i]["AIDVB"].ToString() == "")
                        ExpresionesParseadas[0] = "D@1 " + voucher.Rows[i]["CriptogramaVB"];
                    else
                        ExpresionesParseadas[0] = "I@1 " + voucher.Rows[i]["CriptogramaVB"];

                    RealizarAccion("");
                    ExpresionesParseadas[0] = "AID: " + voucher.Rows[i]["AIDVB"].ToString();
                    RealizarAccion("");
                }
                ...
                {
                    bBandera = true;
                    ExpresionesParseadas[0] = "Pagado con Ptos BCR: " + Double.Parse(voucher.Rows[i]["MontoPagadoConPuntosVB"].ToString()).ToString("###,##0") + " puntos";
                    RealizarAccion("");
                }
                if (bBandera)
                {
                ...
                {
                    ExpresionesParseadas[0] = "PUNTOS BANCOMER";
                    RealizarAccion("");
                    ExpresionesParseadas[0] = "Saldo Anterior Puntos: " + Double.Parse(voucher.Rows[i]["SaldoAnteriorPuntosVB"].ToString()).ToString("###,##0");
                    RealizarAccion("");
                    ExpresionesParseadas[0] = "Importe en pesos: $" + Double.Parse(voucher.Rows[i]["ImportePesosAnteriorVB"].ToString()).ToString("###,##0.00");
                    RealizarAccion("");
                    ExpresionesParseadas[0] = "Redimido Ptos: " + Double.Parse(voucher.Rows[i]["SaldoRedimidoPuntosVB"].ToString()).ToString("###,##0");
                    RealizarAccion("");
                    ExpresionesParseadas[0] = "Importe en pesos: $" + Double.Parse(voucher.Rows[i]["ImporteRedimidoPesosVB"].ToString()).ToString("###,##0.00");
                    RealizarAccion("");
                    ExpresionesParseadas[0] = "Saldo actual Ptos: " + Double.Parse(voucher.Rows[i]["SaldoActualPuntosVB"].ToString()).ToString("###,##0");
                    RealizarAccion("");
                    ExpresionesParseadas[0] = "Importe en pesos: $" + Double.Parse(voucher.Rows[i]["SaldoPuntosImportePesosVB"].ToString()).ToString("###,##0.00");
                    RealizarAccion("");
                    ImprimirTexto("");
                }
            ...
            else  //Tarjeta de crédito Melody-Milano
            {
                ...
                ImprimirTexto(voucher.Rows[i]["CodigoRespuestaVB"] + " MESES CUOTA FIJA");
